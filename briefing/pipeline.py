...
from briefing.images import prepare_section_visuals, validate_visual_runtime
...
def run_pipeline(input_path: Path, out_dir: Path, config: AppConfig) -> BriefingPlan:
    ...
    validate_visual_runtime(plan.sections, config)
    ...
    visual_assets = prepare_section_visuals(plan.sections, visuals_dir, config, config.slides)

    for index, section in enumerate(plan.sections, start=1):
        slide_path = render_section_slide(
            section,
            index,
            slides_dir / f"section_{index:02d}.png",
            config.slides,
            visual_asset_path=visual_assets.get(index),
        )
...
def _allocate_section_durations(
    audio_durations: list[float], target_seconds: int, extra_visual_seconds: float
) -> list[float]:
    base_total = sum(audio_durations) + extra_visual_seconds
    if not audio_durations or base_total >= target_seconds:
        return audio_durations
    # Keep only a short visual tail after narration. The briefing should reach
    # 3-5 minutes through spoken content, not long padded silence.
    padding = min(1.5, (target_seconds - base_total) / len(audio_durations))
...
def _write_cost_report(path: Path, plan: BriefingPlan, final_path: Path) -> None:
    ...
    lines = [
        ...
        "## API justification",
        (
            "The target local machine is an RTX 4060 with 8GB VRAM. Provider-backed still visuals "
            "are the only stage routed to an API because final media polish is the least practical "
            "stage to run locally in this environment. All non-visual-provider stages "
            "remain local."
        ),
        "",
        "## Marginal cost",
        ...
        "provider-generated still imagery, which is isolated and capped.",
        "",
        "## Visual status",
    ]
    for index, section in enumerate(plan.sections, start=1):
        lines.append(f"- section_{index:02d}: {section.visual_mode}")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
